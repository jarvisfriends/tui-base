...
func TestNavigatorGoldenRenders(t *testing.T) {
	...
	testutil.Golden(t, "tabs_60w", tabs.View().Content)
	...
	testutil.Golden(t, "tabs_overflow_30w", wide.View().Content)

	top := NewMinimalTopNav()
	top.SetShowNumbers(true)
	_, _ = top.Update(tea.WindowSizeMsg{Width: 60, Height: 24})
	testutil.Golden(t, "topnav_numbers_60w", top.View().Content)

	sb := New()
	_, _ = sb.Update(tea.WindowSizeMsg{Width: 24, Height: 12})
	testutil.Golden(t, "sidebar_24w", sb.View().Content)
}
