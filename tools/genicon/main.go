...
// renderSVG rasterizes an SVG file into a size x size RGBA image. It renders at
// ss× the target size and then downsamples with a Catmull-Rom filter, because
// rasterizing thin strokes directly at 16–48 px aliases badly (the icon Windows
// actually shows in Explorer and the taskbar). Supersampling gives the tiny
// sizes clean, legible edges.
func renderSVG(path string, size, ss int) (*image.RGBA, error) {
	...
	raster := rasterx.NewDasher(hi, hi, scanner)
