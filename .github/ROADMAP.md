...
| 🔄 In Progress | SP-7 | Directory/File pickers + Multi-file/Multi-directory pickers → `snap/pickers` | **Snap side landed 2026-07-09** (branch `pickers`): DirPicker + MultiFileEditor moved with injected Styles, a HuhTheme hook, and a CollapsePath hook (replacing envpath display shortening); component tests moved, settings-integration tests stay here. Remaining: merge + tag snap, then flip tui-base's settings to import `snap/pickers` (map theme→Styles/HuhThemeFunc/envpath.Collapse) and delete the local copies. |
