...
| ⬜ Planned | I-12 | **Keyboard binding to open the notification history panel** | Found 2026-07-10 while scripting the notifications tape: the history panel only opens by clicking the status bar's notifications region (`status.ClickRegionMsg`) — no key reaches it, violating snap's "every interactive element works keyboard-only" rule (once open it is fully keyboard-driven). Needs a binding in snap `keys.AppKeyMap` (next snap tag) plus a router `key.Matches` case; also lets VHS tapes demo the panel. |
