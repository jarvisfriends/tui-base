...
| 🔄 In Progress | I-12 | **Keyboard binding to open the notification history panel** | Found 2026-07-10 while scripting the notifications tape: the history panel only opens by clicking the status bar's notifications region (`status.ClickRegionMsg`) — no key reaches it, violating snap's "every interactive element works keyboard-only" rule (once open it is fully keyboard-driven). Snap half done 2026-07-10 on the `history-key` branch: `keys.AppKeyMap.ToggleHistory` (default `ctrl+n`, rebindable, in BindingDefs). Remaining after that merges + tags: router `key.Matches(keyMsg, m.keys.ToggleHistory)` case toggling `m.status.ToggleNotifications()` (+ resize cmd), and optionally extend the notifications tape to show the panel. |
