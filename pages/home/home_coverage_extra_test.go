...
func TestOpenBrowserCmdSuccessAndFailure(t *testing.T) {
	fakeOpener(t)
	...
	t.Setenv("PATH", t.TempDir()) // empty PATH dir: opener is missing now
	msg = openBrowserCmd("https://example.invalid/theme")()
	if opened, ok = msg.(browserOpenedMsg); !ok || opened.Err == nil {
		t.Fatalf("missing opener should report an error, got %#v", msg)
	}
}

func TestOpenPathCmdSuccessAndFailure(t *testing.T) {
	fakeOpener(t)
